...
[
	 ...
	 var filler_question = ["Op wie of wat had het voornaamwoord (hij,ze,hem,haar) betrekking?"];
	 var filler_block = {
			type: "survey-text",
			questions: [filler_question],
	 };
...
function processData(filedata, shuffledStructure)
{
	 ...
	 // Input column names
	 //results.push([
	 //			 "name", "native_language", "language_5", "grow_up", "gender", "age", "email", "sentence_id", "sentence_variant","ppos", "RT_ppos", "RT_ppos1", "RT_ppos2", "errouneous word id's"
	 //])
			// Process every sentence
			for (i=0; i<sentenceData.length; ++i)
			{
				 // Check were the errors were made (in a string)
				 var errors = "";
				 ...
				 // Save pronount position
				 sentenceResult.push(shuffledStructure[i].ppos);
				 ...
				 // Save the errouneous word id's
				 sentenceResult.push(errors);
...
$(document).ready(function() {
	 ...
	 $.getJSON(sentenceFile, function(sentences) {
			...
			for (i=0; i<sentences.length/4; i++) {
				 ...
				 // Only hij,ze,hem,haar are pronouns
				 var ppos= 
						(correct.indexOf("hij") != -1) ? correct.indexOf("hij") :
						(correct.indexOf("ze") != -1) ? correct.indexOf("ze") :
						(correct.indexOf("hem") != -1) ? correct.indexOf("hem") :
						(correct.indexOf("haar") != -1) ? correct.indexOf("haar") : 0;
			...
			var experimentStructure = []
				 for (i=0; i<shuffledStructure.length; ++i) {
						...
						for (j=0; j<shuffledStructure[i].correct.length; ++j) {
							 var stimuliText = 
									"<div id='progress'>" +
									"<div id='sentence'>Zin "+ (i+1) + " van " + shuffledStructure.length + "</div>" + 
									"<div id='word'>		Woord " 		+ (j+1) + " van " + shuffledStructure[i].correct.length + "</div>" +
									"</div>";  
							 if (Math.random() > 0.5) {
									stimuliText +=
